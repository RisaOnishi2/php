...
for ($event_count = 1; $event_count <= 10; $event_count++) {
    ...
    if ($asset >= 2000) {
        // 発生するイベントのプログラム
        echo "" . PHP_EOL;
        echo "{$asset}万円貯まった。何か買ってみようか？" . PHP_EOL;
        echo "1: 車を買う。" . PHP_EOL;
        ...
        $choice = trim(fgets(STDIN));
            ...
            switch ($choice) {
                // 入力した値に基づいて実行する処理プログラム
                case 1: echo "車を購入する。1000万円はらう。";
                        $asset = $asset - 1000;
                        $car = $car + 1;
                        break;
                case 2: echo "パソコンを購入する。100万円はらう。";
                        $asset = $asset - 100;
                        $pc = $pc + 1;
                        break;
                case 3: echo "全資産で家を購入する。";
                        $asset = 0;
                        $house = $house + 1;
                        break;
                default: echo "全資産で家を購入する。";
                        $asset = 0;
                        $house = $house + 1;
                        break;
            }
